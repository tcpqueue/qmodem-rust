...
**当前是开发版本，尚不能完整替代 QModem。** 已有原生串口调度、首批厂商操作、HTTP API、鉴权、TOML 设置、日志等级、网卡绑定和 SQLite 初始结构。已接入队列监控页面和频段锁定/SIM 切换；其余后台页面、拨号、短信业务、锁小区及部分适配仍在迁移，见 [功能清单](docs/parity.md)。
